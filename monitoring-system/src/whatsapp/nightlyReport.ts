...
export const initNightlyReportCron = (messageMonitor: MessageMonitor) => {
	...
	const createCompanyCron = async (company: any) => {
		try {
			...
			// Если нужно запустить сегодня, создаем крон с немедленным запуском
			const job = shouldRunToday
				? new CronJob(
						cronExpression,
						async () => {
							console.log(
							...
							try {
								...
								const stats: IReportStats = {
									...
									respondedChats: chats.filter(
										chat => chat.sendMessage === true
									).length,
									unansweredChats: 0,
									avgResponseTime: 0,
								}
				...
				: new CronJob(
						...
						null,
						false, // don't start job right now
						'Asia/Almaty' // timezone
				  )
