...
export const saveCompanySettings = async (req: Request, res: Response) => {
	try {
		...
		if (settings && typeof settings._id === 'object' && settings.nameCompany) {
			const companyId = (settings._id as Types.ObjectId).toString();
	...
	try {
		...
		if (!settings.telegramInviteLink) {
			...
			await telegramService.createGroupsForCompanies([{
				...
				nameCompany: (settings as any).nameCompany
			}])
		}
