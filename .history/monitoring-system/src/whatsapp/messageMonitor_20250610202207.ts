...
export class MessageMonitor {
	...
	public async handleAdminMessage(message: Message): Promise<void> {
		try {
			...
			if (isTelegramCode) {
				...
				// Находим магазин с таким кодом
				const store = await CompanySettings.findOne({ verificationCode: message.body });
				if (!store) {
					console.log(`[${timestamp}] 🔑 Магазин с таким кодом не найден`);
					return;
				}

				// Генерируем invite link
				const inviteLink = await this.telegramService.generateInviteLink(message.chatId, store.nameCompany || 'Store Group');
				
				// Верифицируем группу
				await this.telegramService.verifyGroupCode(message.body, message.chatId, inviteLink);
	...
	private async handleTelegramVerification(message: string, chatId: string): Promise<void> {
		try {
			...
			// Генерируем invite link
			const inviteLink = await this.telegramService.generateInviteLink(chatId, store.nameCompany || 'Store Group');
			
			// Верифицируем группу
			await this.telegramService.verifyGroupCode(code, chatId, inviteLink);
