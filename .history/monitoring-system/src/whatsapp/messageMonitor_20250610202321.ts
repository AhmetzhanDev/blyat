	...
	public async handleAdminMessage(message: Message): Promise<void> {
		try {
			...
			if (isTelegramCode) {
				...
				const inviteLink = await this.telegramService.generateInviteLink(message.from, store.nameCompany || 'Store Group');
				
				// Верифицируем группу
				await this.telegramService.verifyGroupCode(message.body, message.from, inviteLink);
